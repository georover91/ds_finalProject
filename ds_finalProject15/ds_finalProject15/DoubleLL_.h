...
// Class cnostructor & Class destructor & Class copy constructor & MakeEmpty //////////////////
...
DoubleLL<_ItemType>::DoubleLL()
{
	...
	headerPtr->info = "all data";		//이 쌍따옴표 넣으려면 왜 오류 나지?
...
template <class _ItemType>
void DoubleLL<_ItemType>::Print_operKey_history()
//처음위치부터 프린트 하거나, currentPos 위치의 다음 위치부터 프린트함.
{
	if (currentPos != headerPtr) {
		DoubleLLNode<_ItemType>* tempPtr = headerPtr;
		//cout << tempPtr->info;
		//cout << " ";
		...
		while (tempPtr != currentPos) {
			...
			if (tempPtr->info != "\0") {
				...
				cout << "\"";
				cout << " ";
			} //tempPtr->->info == "\0" 이면 key print 안하기
		}
